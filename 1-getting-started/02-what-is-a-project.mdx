...
<Info>
  On paid plans you can also connect a **custom domain** (for example `feedback.yourdomain.com`) instead of the default subdomain. See [Custom domain](/3-configure/06-custom-domain) for setup instructions.
</Info>
...
The panel opens as a right-side sheet with accordion sections. Each section has a dedicated page in [Configure](/3-configure/01-primary-settings):
...
Continue to [Creating and managing projects](/1-getting-started/03-creating-and-managing-projects) to learn how to create, edit, open, and delete projects from the project list.
